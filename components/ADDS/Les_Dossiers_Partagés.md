...
Ce processus attribue automatiquement à chaque utilisateur de l'OU
OU=Utilisateurs,OU=Paris,$DomainDN un dossier personnel sur le serveur
de fichiers, avec les permissions NTFS adaptées, puis relie ce dossier
à son compte AD en tant que lecteur réseau personnel (I:).
...
Module PowerShell ActiveDirectory installé sur le poste d'exécution.
Droits d'écriture sur le partage réseau \\XTS-417.Xtech.green\Partage_Individuel.
Droits de modification des attributs AD (HomeDirectory, HomeDrive) sur les comptes ciblés.
Module XTechLogging.psm1 présent dans C:\Scripts\Modules\.
